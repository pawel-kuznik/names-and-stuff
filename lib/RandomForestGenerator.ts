...
/**
 * Generator for random forest names based on culture
 */
export class RandomForestGenerator implements NameGenerator {
    ...
    /**
     * Creates a new RandomForestGenerator
    ...
    private async ensureInitialized(): Promise<void> {
        if (!this.initialized) {
            const data = await this.dataProvider.getDataArray(this.culture, Feature.Forest);
    ...
    /**
     * Generates a random forest name for the specified culture
     * @returns A randomly selected forest name
     */
